...
pmat1 <- matrix(nrow=4, ncol=4)
tt <- table(geartype_edit$Gear_edit)
types_n <- paste0(names(tt), " (n=", unname(tt), ")")
types <- sort(unique(geartype_edit$Gear_edit))
rownames(pmat1) <- colnames(pmat1) <- types_n
...
diffmat3 <- ifelse(is.na(diffmat2), "", 
                paste("Est =", round(diffmat2, 2), "mm")) # diff in means 
SEmat3 <- ifelse(is.na(SEmat2), "", 
                paste("SE =",round(SEmat2, 2), "mm")) # diff in means 
