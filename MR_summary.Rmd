...
Growth between events was tested using the recorded lengths of recaptured during both events.  A one-tailed paired t-test yielded a p-value of `r growth_pval` (t = `r growth_tstat` on `r growth_df` degrees of freedom), indicating no evidence of positive growth.  The estimated growth was, in fact, negative, with an estimate of `r growth_est` mm FL (SE `r growth_se` mm FL).  This could be interpreted as evidence of slight measurement bias, likely during the winter.  The results presented here do not account for growth; however, the inferences of a growth-corrected estimate are compared at the end of this document.  Instead of physical growth, a "corrected" estimate could be interpreted as correcting for possible measurement bias.
...
pmat3 <- ifelse(is.na(pmat2), "",  ## "--"
                ifelse(pmat2 < 0.00001, "p < 0.00001**",
                       paste("p =",ifelse(pmat2 < a_sidak, paste0(format(round(pmat2, 5), scientific=FALSE), "**"),
                              round(pmat2, 3)))))
Dmat3 <- ifelse(is.na(Dmat2), "", 
                paste("D =",round(Dmat2, 2)))
diffmat3 <- ifelse(is.na(diffmat2), "", 
                paste("Est diff in means =", round(diffmat2, 2), "mm"))
SEmat3 <- ifelse(is.na(SEmat2), "", 
                paste("SE =",round(SEmat2, 2), "mm"))
